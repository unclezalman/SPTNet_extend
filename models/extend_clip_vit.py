...
class extend_CLIP(nn.Module):
    ...
    def build_attention_mask_get(self):
        # lazily create causal attention mask, with full attention between the vision tokens
        # pytorch uses additive attention mask; fill with -inf
        mask = torch.empty(self.n_ctx, self.n_ctx)
        mask.fill_(float("-inf"))
        mask.triu_(1)  # zero out the lower diagonal
        return mask
    ...
    def prepare_pseudo_text_tensor(self, pseudo_tokens):
        ...
        end_tokens_ids = (torch.ones(num_preds)*(num_words-1)).long()

        return pseudo_tokens, end_tokens_ids
...
def build_extend_model(state_dict: dict, n_ctx: int=None, use_proj: bool=True):
    vit = "visual.proj" in state_dict
